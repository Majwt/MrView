...
public class GraphReadRepository : IGraphReadRepository
{
    ...
    public async Task<IEnumerable<EdgeEntity>> getEdgesAsync(GraphCursor cursor, GraphQueryParams queryParams)
    {
        ...
        var activeFilter = queryParams.ManagedOnly
            ...
            : "";
        var staleEdgeFilter = queryParams.MinLastSeenHours.HasValue
            ? "AND e.last_seen >= DATEADD(HOUR, -@MinLastSeenHours, GETUTCDATE())"
            : "";
        var sql = $"""
            SELECT
                e.id,
    ...
    public async Task<IEnumerable<NodeSummaryEntity>> getNodeSummariesAsync(GraphCursor cursor, GraphQueryParams queryParams)
    {
        ...
        var staleFilter = queryParams.MinLastSeenHours.HasValue
            ? "AND n.last_seen >= DATEADD(HOUR, -@MinLastSeenHours, GETUTCDATE())"
            : "";
        var managedHavingFilter = queryParams.ManagedOnly
            ? "AND MAX(e.endpoint_a_ciid) IS NOT NULL AND MAX(e.endpoint_b_ciid) IS NOT NULL"
            : "";
        ...
        int? scopeCustomerId = null)
    {
        ...
        if (firstSeenAfter.HasValue)
        {
            command.Parameters.Add(new SqlParameter("@FirstSeenAfter", System.Data.SqlDbType.DateTime2)
            {
                Value = firstSeenAfter.Value,
            });
        }

        if (firstSeenBefore.HasValue)
        {
            command.Parameters.Add(new SqlParameter("@FirstSeenBefore", System.Data.SqlDbType.DateTime2)
            {
                Value = firstSeenBefore.Value,
            });
        }

        if (lastSeenAfter.HasValue)
        {
            command.Parameters.Add(new SqlParameter("@LastSeenAfter", System.Data.SqlDbType.DateTime2)
            {
                Value = lastSeenAfter.Value,
            });
        }

        if (lastSeenBefore.HasValue)
        {
            command.Parameters.Add(new SqlParameter("@LastSeenBefore", System.Data.SqlDbType.DateTime2)
            {
                Value = lastSeenBefore.Value,
            });
        }
